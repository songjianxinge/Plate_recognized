...
namespace easypr {
	...
	void deleteNotArea(Mat &inmat, Color color = UNKNOWN) {
		...
		if (bFindLeftRightBound2(img_threshold, posLeft, posRight)) {
		...
		for (size_t i = 0; i < inRects.size(); i++) {
			...
			if (roi_angle - m_angle < 0 && roi_angle + m_angle > 0) {
				...
				if (deskew_mat.cols * 1.0 / deskew_mat.rows > 2.3 && deskew_mat.cols * 1.0 / deskew_mat.rows < 6) {
					...
					plate.plate_rect = safeBoundRect;
...
#pragma omp section
			{
				...
				//imshow("plates_blue", plates_blue);
			}
#pragma omp section
			{
	...
	int CPlateLocate::plateMserLocate(Mat src, vector<CPlate> &candPlates, int img_index) {
		...
		for (size_t i = 0; i < channelImages.size(); ++i) {
			...
			Mat image = scaleImg(channelImage, Size(scale_size, scale_size), scale_ratio);
			...
			for (size_t j = 0; j < flags.size(); j++) {
				...
				for (auto plate : plates) {
					...
					RotatedRect scaleRect = scaleBackRect(rrect, (float)scale_ratio);
...
//					this->plateSobelLocate(src, sobel_Plates, index);
//				}
//			}
//#pragma omp section
//			{
//				if (!m_type || m_type & PR_DETECT_COLOR) {
//					this->plateColorLocate(src, color_Plates, index);
//				}
//			}
//#pragma omp section
//			{
//				if (!m_type || m_type & PR_DETECT_CMSER) {
//					this->plateMserLocate(src, mser_Plates, index);
//				}
//			}
//		}
//
	...
	/*	for (auto plate : sobel_Plates) {
		...
		/*CPlateJudge::instance()->plateJudgeUsingNMS(all_result_Plates, resultVec, 3);

	if (0)
			showDectectResults(src, resultVec, 3);*/
		return 0;
	}
...
*/
	/*	for (size_t i = 0; i < all_result_Plates.size(); i++) {
		...
		// use nms to judge plate
		CPlateJudge::instance()->plateJudgeUsingNMS(all_result_Plates, resultVec, m_maxPlate);
		
		if (1)
			showDectectResults(src, resultVec, m_maxPlate);
		return 0;
	}

...
namespace easypr {
	...
	bool bFindLeftRightBound3(Mat &bound_threshold, int &posLeft, int &posRight) {
...
#define VERTICAL 0
	...
	Mat scaleImg(const Mat &image, const Size &maxSize, double &scale_ratio) {
		Mat ret;
	...
	RotatedRect scaleBackRect(const RotatedRect &rr, const float scale_ratio) {
	...
	void slideWindowSearch(const Mat &image, std::vector<CCharacter> &slideCharacter, const Vec4f &line,
		...
		for (int slideX = -slideLength; slideX < slideLength; slideX += slideStep) {
			...
			cv::threshold(region, binary_region, ostu_level, 255, THRESH_BINARY);
			//double ostu_level = threshold(region, binary_region, 0, 255, THRESH_BINARY | THRESH_OTSU);
	...
	bool judegMDOratio2(const Mat &image, const Rect &rect, std::vector<Point> &contour, Mat &result, const float thresh,
		...
		Mat mser = image(rect);
		Mat mser_mat;
		cv::threshold(mser, mser_mat, 0, 255, THRESH_BINARY | THRESH_OTSU);

		Rect normalRect = adaptive_charrect_from_rect(rect, image.cols, image.rows, useExtendHeight);
		Mat region = image(normalRect);
		Mat thresh_mat;
		cv::threshold(region, thresh_mat, 0, 255, THRESH_BINARY | THRESH_OTSU);
	...
	void removeRightOutliers(std::vector<CCharacter> &charGroup, std::vector<CCharacter> &out_charGroup, double thresh1,
		...
		for (size_t charGroup_i = 0; charGroup_i + 1 < charGroup.size(); charGroup_i++) {
			...
			fitLine(Mat(two_points), line_btp, DIST_L2, 0, 0.01, 0.01);
...
#pragma omp parallel for
		for (int color_index = 0; color_index < 2; color_index++) {
			...
			for (size_t index = 0; index < size; index++) {
				...
				if (verifyCharSizes(rect)) {
					...
					Mat tmpMat;
					double ostu_level = cv::threshold(image(charRect), tmpMat, 0, 255, THRESH_BINARY | THRESH_OTSU);
			...
			for (auto plate : plateVec) {
				...
				if (mserCharacter.size() < char_max_count) {
					...
					if (1) {
						...
						ostu_level = cv::threshold(region, binary_region, 0, 255, THRESH_BINARY | THRESH_OTSU);
						if (1 && showDebug) {
	...
	void spatial_ostu(InputArray _src, int grid_x, int grid_y, Color type) {
		...
		for (int i = 0; i < grid_y; i++) {
			for (int j = 0; j < grid_x; j++) {
				...
				if (type == BLUE) {
					cv::threshold(src_cell, src_cell, 0, 255, THRESH_OTSU + THRESH_BINARY);
				}
				else if (type == YELLOW) {
					cv::threshold(src_cell, src_cell, 0, 255, THRESH_OTSU + THRESH_BINARY_INV);
				}
				else if (type == WHITE) {
					cv::threshold(src_cell, src_cell, 0, 255, THRESH_OTSU + THRESH_BINARY_INV);
				}
	...
	Mat uniformResize(const Mat &result, float &scale) {
		...
		if (nCols <= img_window.cols && nRows <= img_window.rows) {
			...
			resize(result, result_resize, Size(), scale, scale, INTER_AREA);
	...
	void showDectectResults(const Mat &img, const vector<CPlate> &plateVec, size_t num) {
		...
		if (1) {
			...
			for (size_t j = 0; j < plateVec.size(); j++) {
				...
				for (int j = 0; j < 4; j++)
					line(result, rect_points[j], rect_points[(j + 1) % 4], lineColor, 3, 8);
